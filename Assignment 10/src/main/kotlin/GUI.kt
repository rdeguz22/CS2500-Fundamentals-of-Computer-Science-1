...
class GUI(val humanPlayer: Boolean) : UserInterface {
    var turn = 0
    ...
    val keyStates: MutableMap<String, KeyState> = mutableMapOf<String, KeyState>()
        .withDefault { key ->
            when {
    ...
    override fun showWin(numGuesses: Int) {
        display(WordleGame.WINNING_RESPONSES[turn - 1], black)
        gameOver = true
    }
    ...
    private fun update(s: String, newState: KeyState) {
        ...
        if (newState != oldState) {
            val button = gridButtonMap[s] ?: throw AssertionError("No entry in buttonMap for '$s'")
            when (newState) {
                KeyState.Right -> {
...
fun main(args: Array<String>): Unit = application {
    ...
    program {
        val keyboardRows = listOf(
            ...
            listOf("5", ENTER_LABEL, "Z", "X", "C", "V", "B", "N", "M", BACKSPACE_LABEL)
        )
        ...
        keyboard.keyDown.listen {
            ...
            if (!gui.gameOver && gui.humanPlayer) {
                when (it.name) {
                    "enter" -> processText(ENTER_LABEL)
                    "backspace" -> processText(BACKSPACE_LABEL)
                    else -> if (it.name.length == 1) processText(it.name[0].uppercase()) else System.err.println("Can't handle ${it.name}")
                }
            }
        }
        ...
        val cm = controlManager {
            ...
            layout {
                ...
                val gridDivs = 0.until(MAX_TURNS).map { _: Int ->
                    div("grid") {
                        ...
                        row.forEachIndexed { _, word ->
                            button {
                                ...
                                style = styleSheet {
                                    ...
                                    background = gui.keyStates.getValue(word).color
                                    width = if (word.isNumber()) word.toInt().px else
                                        when (word.length) {
                                            5 -> 50.px
                                            2 -> 35.px
                                            else -> 25.px
                                ...
                                events.clicked.listen {
                                    if (gui.humanPlayer && !gui.gameOver) {
                ...
                run {
                    ...
                    gui.grid = gridDivs.map { gridDiv: Div ->
                        gridDiv.children.filterIsInstance<Button>().filter { it.label.isEmpty() }
                    }
                    val buttons: List<Button> = keyboardDivs.flatMap { div ->
                        div.children.filterIsInstance<Button>()
                    }
                    gui.gridButtonMap = buttons.associateBy { button -> button.label }
                }
            }
        }
        extend(cm)
    ...
    run {
        try {
            val human = isHumanPlayer(args)
            thread(start = true) {
                gui = GUI(human)
                val player = if (human)
                    HumanPlayer(gui)
                else
                    ArtificialPlayer(gui)
                playGame(player, gui)
            }
