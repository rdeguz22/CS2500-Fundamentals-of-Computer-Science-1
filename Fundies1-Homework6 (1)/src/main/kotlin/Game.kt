...
object Game {
    ...
    fun getEntity(
        ...
        if (x in 0.until(NUM_COLS) && y in 0.until(NUM_ROWS)) {
            entityGrid[x][y]
        } else {
            null
        }
    ...
    fun calculateDistance(
        ...
        if (position1 == null) throw IllegalArgumentException("This living entity is not on the board.")
        val position2 = getPosition(entity2)
        if (position2 == null) throw IllegalArgumentException("This living entity is not on the board.")
        return sqrt((position1.x - position2.x) * (position1.x - position2.x) + (position1.y - position2.y) * (position1.y - position2.y).toDouble())
    }
